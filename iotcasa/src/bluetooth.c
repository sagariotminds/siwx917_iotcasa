...
osThreadId_t BleAppHandle = NULL;
...
void stop_ble_service(void)
{
    ...
    if (BleAppHandle != NULL) {
        status = osThreadTerminate(BleAppHandle);

        if (status == SL_STATUS_OK ) {
            LOG_INFO("BLE", "BLE Thread deleted successfully.");
            BleAppHandle = NULL; // Reset handle to prevent double-deletion
        } else {
            LOG_ERROR("BLE", "Failed to delete BLE Thread: %d", status);
        }
    }
...
static void rsi_ble_add_char_serv_att(void *serv_handler, uint16_t handle, uint8_t val_prop, uint16_t att_val_handle, uuid_t att_val_uuid)
{
...
static void rsi_ble_add_char_val_att(void *serv_handler, uint16_t handle, uuid_t att_type_uuid, uint8_t val_prop,
                                     uint8_t *data, uint8_t data_len, uint8_t config_bitmap)
{
    rsi_ble_req_add_att_t new_att = { 0 };
...
sl_status_t casa_ble_init(void)
{
    ...
    BleAppHandle = osThreadNew(casa_ble_process, NULL, &ble_app_thread_attributes);
    if (BleAppHandle != NULL) {
        LOG_INFO("BLE", " BLE task created.");
    } else {
        LOG_ERROR("BLE", "BLE  task not created.");
    }
    return status;
}
